...
const elementDatabase = {
  elements: ['grass', 'fire', 'water', 'ice', 'bug', 'normal',
   'fighting', 'psychic', 'ghost', 'poison', 'dark', 'electric',
   'steel', 'dragon', 'rock', 'ground', 'fairy', 'flying', ''],

  // when a pokemon with a weakness attacks, it will deal less damage
  // to an opposing pokemon.
  weaknesses: {
      grass: ['grass', 'fire', 'ice', 'poison', 'dragon', 'flying'],      // grass
      fire: ['water', 'fire', 'rock', 'ground', 'dragon'],                // fire
      water: ['water', 'grass', 'electric', 'dragon'],                    // water
      ice: ['fire', 'rock', 'steel'],                                     // ice
      bug: ['rock', 'fire', 'flying'],                                    // bug
      normal: ['rock', 'steel', 'ghost'],                                 // normal
      fighting: ['psychic', 'bug', 'flying', 'ghost', 'fairy'],           // fighting
      psychic: ['psychic', 'ghost', 'dark', 'steel', 'bug'],              // psychic
      ghost: ['dark', 'steel', 'normal'],                                 // ghost
      poison: ['poison', 'psychic', 'ghost', 'rock', 'ground', 'steel'],  // poison
      dark: ['dark', 'fighting', 'steel', 'bug'],                         // dark
      electric: ['electric', 'grass', 'ground', 'dragon'],                // electric
      steel: ['steel', 'water', 'fighting', 'fire', 'ground'],            // steel
      dragon: ['ice', 'steel', 'fairy'],                                  // dragon
      rock:['rock', 'ground', 'fighting', 'steel', 'grass', 'water'],     // rock
      ground:['grass', 'water', 'ice', 'bug', 'flying'],                  // ground
      fairy: ['steel', 'poison'],                                         // fairy
      flying: ['ice', 'electric', 'steel', 'rock']                        // flying
   },

  // when a pokemon with a strength attacks, it will deal extra damage.
  strengths: {
      grass: ['water', 'ground', 'rock'],                                 // grass
      fire: ['grass', 'ice', 'bug', 'steel'],                             // fire
      water: ['fire', 'ground', 'rock'],                                  // water
      ice: ['grass', 'ground', 'flying'],                                 // ice
      bug: ['psychic', 'grass', 'dark'],                                  // bug
      normal: [],                                                         // normal
      fighting: ['ice', 'rock', 'steel', 'dark'],                         // fighting
      psychic: ['poison', 'fighting'],                                    // psychic
      ghost: ['ghost', 'poison', 'dark'],                                 // ghost
      poison: ['grass'],                                                  // poison
      dark: ['psychic', 'ghost'],                                         // dark
      electric: ['water', 'flying'],                                      // electric
      steel: ['ice', 'rock'],                                             // steel
      dragon: ['dragon'],                                                 // dragon
      rock: ['fire', 'ice', 'bug'],                                       // rock
      ground: ['ground', 'electric'],                                     // ground
      fairy: ['fighting', 'dragon'],                                      // fairy
      flying: ['grass', 'bug', 'fighting'],                               // flying
  }
...
class Pokemon {
  constructor(PokeName, hP, Att, Def, Spd)
  {
    // let Elements = [twoElements[0], twoElements[1]];
  ...
  //   let thisDamage = 0;
  //
  //   let thisDamage = this.chosenPokemon.Attack - opposingPokemon.Defense;
  //   if(thisDamage < 0) { thisDamage = 1; }
  //
  //   for(let i = 0; i < this.elements.length; i++)
  //   {
  //     for(let j = 0; i < opposingPokemon.elements.length; j++)
  //     {
  //       for(let k = 0; k < elementDB.weaknesses[this.Elements[i]]; k++)
  //       {
  //         // calculate weakness
  //         if(elementDB.weaknesses[this.Elements[i]][k] === opposingPokemon.Elements[j])
  //         {
  //           thisDamage -= 10;
  //         }
  //
  //         // calculate strength
  //         if(elementDB.strengths[this.Elements[i]][k] === opposingPokemon.Elements[j])
  //         {
  //           thisDamage += 10;
  //         }
  //       }
...
class Player {
  constructor(Name, benchPokemon, computerBool)
  {

    let playerName = "";

    let benchedPokemon = [];
    let chosenPokemon = null; // the pokemon to be chosen from
                          // the set of benched pokemon
    let knockedOutPokemon = 0; // counts the number of knocked out pokemon

    let isComputer = computerBool;
    let loser = false;

    for(let i = 0; i < benchPokemon.length; i++)
    {
      benchedPokemon[i] = benchPokemon[i];
    }
  }
  ...
  choosePokemon()
  {
    if(knockedOutPokemon > 0)
    {
      while(this.chosenPokemon == null || chosenPokemon.isKnockedOut === true)
      {
        if(isComputer = false)
        {
          chosenPokemon = prompt("Choose a pokemon to fight with: ");
          console.log("You have chosen : " + chosenPokemon.getName());
        }
        else {
  ...
  addPokemon(thisPokemon) { benchedPokemon.push(thisPokemon); }

  pokemonLost() {
    if(chosenPokemon.knockOut() === true)
    {
      knockedOutPokemon++;
      return true;
    }
  ...
  hasLost() {
    if(knockedOutPokemon === benchedPokemon.length)
    {
      return true;
    }
    else {
